...
export async function get_ramp_up_time_metric(
  url: string,
): Promise<RampUpResult> {
  ...
  try {
    ...
    logger.debug("Retrieved README content", {
    ...
    logger.info("Ramp-up time metric calculation complete", {
      url,
      score,
      latency,
    });
...
function calculateRampUpScore(readmeContent: string): number {
  ...
  logger.debug("Header score calculated", {
    headerCount,
    headerScore: Math.min(headerCount / 5, 0.3),
  });

  // Check for code examples
  const codeBlockCount = (readmeContent.match(/```[\s\S]*?```/g) || []).length;
  score += Math.min(codeBlockCount / 3, 0.2); // Cap at 0.2 for code blocks
  logger.debug("Code block score calculated", {
    codeBlockCount,
    codeBlockScore: Math.min(codeBlockCount / 3, 0.2),
  });
