...
export async function getCorrectnessMetric(
  gitHubUrl: string,
...
function calculateCorrectnessScore(
  ...
  const weightedScore =
    issueResolutionRate * issueWeight + prMergeRate * prWeight;

  // Apply a logarithmic scale to favor repositories with more activity
  const activityFactor =
    Math.log10(totalIssues + totalPRs + 1) / Math.log10(101); // Normalize to 0-1 range
