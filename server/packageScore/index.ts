  ...
  if (repoInfo) {
    try {
      const cloneDir = path.join(
        process.cwd(),
        "cloned_repos",
        `${repoInfo.owner}_${repoInfo.repo}`,
      );
...
program.version("1.0.0").description("ACME Module Trustworthiness CLI");
program;
program
        ...
        try {
          ...
          const formattedResult = {
            ...
            NetScore_Latency: parseFloat(
              (result.NetScore_Latency / 1000).toFixed(3),
            ),
            RampUp: parseFloat(result.RampUp.toFixed(3)),
            RampUp_Latency: parseFloat(
              (result.RampUp_Latency / 1000).toFixed(3),
            ),
            ...
            ResponsiveMaintainer: parseFloat(
              result.ResponsiveMaintainer.toFixed(3),
            ),
            ...
            License_Latency: parseFloat(
              (result.License_Latency / 1000).toFixed(3),
            ),
            ...
            DependencyNetric_Latency: parseFloat(
  ...
  .action(() => {
    ...
    jestProcess.on("close", () => {
      ...
      if (!checkFileExists(coverageSummaryPath)) {
        console.error(
          "Coverage summary file does not exist:",
          coverageSummaryPath,
        );
        return;
      }
      ...
      try {
        ...
        console.log(
          `${results.numPassedTests}/${results.numTotalTests} test cases passed. ${lineCoverage}% line coverage achieved.`,
        );
      } catch (error) {
