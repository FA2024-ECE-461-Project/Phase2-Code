...
export function test_API(): void {
  ...
  const getPullRequestCount = async () => {
    try {
      ...
      logger.info(`Number of pull requests: ${response.data.length}`, {
        owner: OWNER,
        repo: REPO,
      });
    } catch (error) {
      logger.error("Error fetching pull requests", {
...
export async function getClosedPRs(
  ...
  const response = await axios.get(apiUrl, { headers });
  logger.debug("Closed PRs fetched", {
    count: response.data.length,
    owner,
    repo,
  });
...
export async function getNpmPackageGitHubUrl(
  ...
  try {
    ...
    if (repoUrl) {
      ...
      logger.debug("GitHub URL fetched for NPM package", {
        packageName,
        cleanUrl,
      });
      return cleanUrl;
    ...
    logger.error("Error fetching NPM package info", {
      packageName,
      error: (error as Error).message,
    });
    return null;
...
export async function getReadmeContent(
  ...
  try {
    ...
    const readmeFile = response.data.find((file) =>
    ...
    const readmeResponse = await axios.get<ReadmeContent>(readmeFile.url, {
      headers,
    });
    ...
    logger.debug("README content fetched successfully", {
      owner,
      repo,
    ...
    logger.error("Error fetching README content", {
      owner,
      repo,
      error: (error as Error).message,
    });
...
export function test_getReadmeContent(): void {
  ...
  getReadmeContent(OWNER, REPO)
    .then((readmeContent) =>
      logger.info("README content fetched", {
        owner: OWNER,
        repo: REPO,
  ...
  try {
    ...
    for (const issue of response.data) {
      if (issue.state === "closed") {
        totalClosureTime += responsive.getTimeDifferenceInHours(
          issue.created_at,
          issue.closed_at,
        );
        totalIssues++;
    ...
    logger.debug("Average closure time calculated", {
      owner,
      repo,
      avgClosureTime,
    });
    ...
    logger.error("Error calculating average closure time", {
      owner,
      repo,
      error: (error as Error).message,
    });
  ...
  try {
    ...
    for (const pull of Pulls.data) {
      ...
      if (comments.length == 0) {
        ...
        total += responsive.getTimeDifferenceInHours(
          pull.created_at,
    ...
    logger.debug("Average response time calculated", {
      owner,
      repo,
      avgResponseTime,
    });
...
export async function getIssues(owner: string, repo: string) {
  ...
  const token = getToken();
  const { headers } = get_axios_params(
  ...
  try {
    const response = await axios.get(apiUrl, { headers });
    logger.debug("Issues fetched", {
      owner,
      repo,
      issueCount: response.data.length,
    });
    return response.data;
  } catch (error) {
...
export async function getOpenIssues(
  ...
  const response = await axios.get(apiUrl, { headers });
  logger.debug("Open issues fetched", {
    owner,
    repo,
    count: response.data.length,
  });
...
export async function getClosedIssues(
  ...
  const response = await axios.get(apiUrl, { headers });
  logger.debug("Closed issues fetched", {
    owner,
    repo,
    count: response.data.length,
  });
...
export async function getCommitsAndContributors(
  ...
  try {
    ...
    logger.error("Error fetching commits and contributors", {
      owner,
      repo,
      error: (error as Error).message,
    });
...
export async function getCodeReviewLines(
  ...
  logger.debug("Calculating number of lines that were code reviewed", {
    owner,
    repo,
  });
  try {
    ...
    for (const pull of Pulls.data) {
      const prData = await axios.get(
        `https://api.github.com/repos/${owner}/${repo}/pulls/${pull.number}`,
        { headers },
      );
    ...
    logger.debug("Calculated values for lines that were code reviewed", {
      owner,
      repo,
      reviewedLinesAdded,
      totalLinesAdded,
      ratioReviewedLinesAdded,
    });
    return ratioReviewedLinesAdded;
  } catch (error) {
    logger.error(
      "Error calculating number of lines added through code reviews",
